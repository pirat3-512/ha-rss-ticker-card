...
- Fixed pixel scroll speed (time adjusts to content length).
- Hover‑to‑pause.
- Per‑headline color cycling.
- Recommended 1680px wide bar via `mod-card`.

This card does **not** fetch RSS itself. It reads from a sensor entity that already exposes arrays of RSS titles and links as attributes.
...
Make sure this repository is public on GitHub, for example:

text
https://github.com/<your-user>/ha-rss-ticker-card
In Home Assistant:
...
Repository: https://github.com/<your-user>/ha-rss-ticker-card

Category: Frontend.

Add the repository, then locate RSS Ticker Card in HACS → Frontend and install it.

After installation, HACS will register the resource automatically.
...
Place this card in a row or view where it has enough horizontal space (for example, a grid row that spans multiple columns, or a single‑column/panel view).
...
It measures that strip’s width and sets the animation duration to width / speed.
...
If you ever change your RSS sensor to use different attribute names, just adjust titles_attr and links_attr in the YAML—no changes to the card code are needed.
